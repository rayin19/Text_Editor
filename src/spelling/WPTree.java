...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		WPTreeNode t = root;
		while(!queue.isEmpty() && !t.getWord().equals(word2)) {
			t = queue.remove(0);
			List<String> c = nw.distanceOne(t.getWord(), true);
			Iterator<String> it = c.iterator();
			while(it.hasNext()) {
				String y = it.next();
				if(!visited.contains(y)) {
					visited.add(y);
					queue.add(t.addChild(y));
				}
			}
		}
		if(t != null && t.getWord().equals(word2)) {
			path = t.buildPathToRoot();
	    return path;
		}
		return null;
	}
